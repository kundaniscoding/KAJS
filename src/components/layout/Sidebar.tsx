...
export function Sidebar({
  ...
  const [isReportsExpanded, setIsReportsExpanded] = useState(view.startsWith("report-"));
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex h-16 items-center border-b border-slate-200 relative transition-all duration-300 overflow-hidden",
              ...
              <TooltipTrigger asChild>
                ...
                >
                  <BarChart3
                    className={cn(
                      "h-4.5 w-4.5 shrink-0 transition-colors",
                      view.startsWith("report-") ? "text-indigo-600" : "text-slate-500",
                    )}
                  />
                  <span
                    className={cn(
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        active ? "bg-indigo-600 scale-125" : "bg-slate-300",
                      )}
                    />
                    {sub.label}
                  </button>
      ...
      <div className="relative border-t border-slate-200 transition-all duration-300 flex flex-col shrink-0 bg-white">
        ...
        >
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                className={cn(
                  "flex items-center gap-3 rounded-xl hover:bg-slate-50 transition-colors w-full text-left",
                  isCollapsed ? "justify-center p-1" : "p-2",
                )}
              >
                <Avatar className="h-9 w-9 shrink-0 ring-2 ring-white shadow-sm">
                  <AvatarFallback className="bg-indigo-600 font-semibold text-xs text-white">
                    AR
                  </AvatarFallback>
                </Avatar>
                {!isCollapsed && (
        ...
        >
          <div className="w-full rounded-xl bg-slate-50 border border-slate-100 p-3 shadow-sm">
            ...
            <div className="mt-1 text-[10px] font-medium text-slate-500 uppercase tracking-widest">
              Running Package: Pro
            </div>
          </div>
        </div>
