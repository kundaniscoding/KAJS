...
import { Sunrise, Sun, Clock, Moon, CalendarRange, Edit, Users, UserCheck, UserX, LayoutList, X } from "lucide-react";
...
const ICON_MAP: Record<string, React.ComponentType<{ className?: string }>> = {
  Sunrise, Sun, Clock, Moon, CalendarRange,
};
...
const TONE_ICON: Record<string, string> = {
  ...
  emerald:"bg-gradient-to-br from-emerald-100 to-emerald-200/60 text-emerald-700 shadow-emerald-100",
...
export function ShiftsView({ searchQuery, shiftsList, setShiftsList }: ShiftsViewProps) {
  ...
  const filteredShifts = useMemo(() => {
    ...
    return shiftsList.filter(s => s.name.toLowerCase().includes(q) || s.time.toLowerCase().includes(q));
  }, [searchQuery, shiftsList]);

  const totalEmployees = 242;
  const totalShifts = shiftsList.length;
  const totalAssigned = useMemo(() => shiftsList.reduce((sum, s) => sum + (s.assigned || 0), 0), [shiftsList]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        {statCards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className="flex items-center gap-4 overflow-hidden rounded-2xl border border-white/60 bg-white/60 px-5 py-4 shadow-sm shadow-slate-200/40 backdrop-blur-xl"
            >
              <div className={cn("grid h-11 w-11 shrink-0 place-items-center rounded-xl shadow-sm ring-1", card.iconCls, card.ringCls)}>
                <Icon className="h-5 w-5" />
              </div>
              <div>
                <div className={cn("text-2xl font-black tracking-tight leading-none", card.valueCls)}>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/60 bg-white/60 shadow-sm shadow-slate-200/50 backdrop-blur-xl">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-100/60 bg-slate-50/40">
              {["Shift", "Hours", "Check-In Window", "Check-Out Window", "Assigned", "Coverage", ""].map(h => (
                <th key={h} className="py-3 px-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider first:pl-6 last:pr-6 last:text-right">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filteredShifts.length === 0 ? (
              ...
              filteredShifts.map((s) => {
                ...
                return (
                  <tr key={s.name} className="border-b border-slate-100/40 last:border-0 hover:bg-slate-50/50 transition-colors group">
                    {/* Shift icon + name */}
                    <td className="py-4 pl-6 pr-4">
                      <div className="flex items-center gap-3">
                        <div className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-xl shadow-sm", TONE_ICON[s.tone] || TONE_ICON.slate)}>
                          <Icon className="h-4 w-4" />
                        </div>
                        <div>
                          <div className="font-bold text-slate-800 text-[13px] leading-tight">{s.name}</div>
                          <div className="text-[10px] text-slate-400 font-medium mt-0.5">Shift</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <div className="font-semibold text-slate-700 tabular-nums">{s.checkIn} – {s.checkOut}</div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <div className="text-[10px] text-slate-400 font-medium">Scheduled window</div>
                        {s.crossDay && (
                    ...
                    <td className="py-4 px-4 min-w-[140px]">
                      ...
                      <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100/80">
                        <div
                          className={cn("h-full rounded-full transition-all duration-700", TONE_BAR[s.tone] || TONE_BAR.slate)}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/45 backdrop-blur-md animate-in fade-in duration-300">
          <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-white/40 bg-white/85 p-6 shadow-2xl backdrop-blur-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between border-b border-slate-200/50 pb-3 shrink-0">
              <div>
                <h2 className="text-base font-bold text-slate-800 tracking-tight">Edit Shift — {editingShift.name}</h2>
                <p className="text-xs text-slate-500 mt-0.5 font-medium">Update shift hours, grace windows and theme</p>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            <form
              onSubmit={(e) => {
                ...
                const crossDay     = (e.currentTarget.elements.namedItem("crossDay") as HTMLInputElement)?.checked ?? false;

                if (!name || !start || !end || !checkInFrom || !checkInUntil || !checkOutFrom || !checkOutUntil) {
            ...
            >
              ...
              <div className="space-y-1.5">
                ...
                <Input name="name" defaultValue={editingShift.name} required className="h-9 border-slate-200 focus-visible:ring-indigo-500 text-xs" />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input type="time" name="start" defaultValue={to24(editingShift.checkIn)} required className="h-9 border-slate-200 focus-visible:ring-indigo-500 text-xs font-semibold" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-medium">End Time</label>
                    <Input type="time" name="end" defaultValue={to24(editingShift.checkOut)} required className="h-9 border-slate-200 focus-visible:ring-indigo-500 text-xs font-semibold" />
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-3.5 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input type="time" name="checkInFrom" defaultValue={to24(editingShift.checkInFrom || editingShift.checkIn)} required className="h-9 border-emerald-200 focus-visible:ring-emerald-500 bg-white text-xs font-semibold" />
                  ...
                  <div className="space-y-1">
                    ...
                    <Input type="time" name="checkInUntil" defaultValue={to24(editingShift.checkInUntil || editingShift.checkIn)} required className="h-9 border-rose-200 focus-visible:ring-rose-500 bg-white text-xs font-semibold" />
                    <p className="text-[9px] text-slate-400 font-medium">Latest accepted check-in</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-3.5 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-semibold">Starts From *</label>
                    <Input type="time" name="checkOutFrom" defaultValue={to24(editingShift.checkOutFrom || editingShift.checkOut)} required className="h-9 border-amber-200 focus-visible:ring-amber-500 bg-white text-xs font-semibold" />
                    <p className="text-[9px] text-slate-400 font-medium">Earliest allowed check-out</p>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-semibold">Stops At *</label>
                    <Input type="time" name="checkOutUntil" defaultValue={to24(editingShift.checkOutUntil || editingShift.checkOut)} required className="h-9 border-indigo-200 focus-visible:ring-indigo-500 bg-white text-xs font-semibold" />
                    <p className="text-[9px] text-slate-400 font-medium">Latest overtime check-out</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select name="tone" defaultValue={editingShift.tone} className="flex h-9 w-full rounded-md border border-slate-200 bg-white px-3 py-1 text-xs shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-indigo-500">
              ...
              <label className="flex items-start gap-3 cursor-pointer rounded-xl border border-slate-200/70 bg-slate-50/50 px-4 py-3 hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors">
                <input type="checkbox" name="crossDay" defaultChecked={!!editingShift.crossDay} className="mt-0.5 h-4 w-4 rounded border-slate-300 accent-indigo-600" />
                <div>
              ...
              <div className="flex items-center justify-end gap-3 border-t border-slate-200/50 pt-4 mt-2">
                <Button type="button" variant="outline" onClick={() => setEditingShift(null)} className="h-9 border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-semibold">
                  Cancel
                </Button>
                <Button type="submit" className="h-9 bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm font-semibold text-xs px-5">
                  Save Changes
                </Button>
              </div>
            </form>
